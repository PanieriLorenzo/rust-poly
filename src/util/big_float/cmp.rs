...
#[cfg(test)]
mod test {
    use crate::util::big_float::F128;

    use super::cmp;

    #[test]
    fn equality() {
        let zero = F128::const_from_f64(0.0);
        assert!(cmp(zero, zero).unwrap().is_eq());

        let neg_zero = F128::const_from_f64(-0.0);
        assert!(cmp(neg_zero, neg_zero).unwrap().is_eq());

        assert!(cmp(zero, neg_zero).unwrap().is_eq());

        let one = F128::const_from_f64(1.0);
        assert!(cmp(zero, one).unwrap().is_ne());
    }
}
