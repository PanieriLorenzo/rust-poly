...
use super::{u128_widen_mul, F128};

type F = F128;
type FInt = u128;

pub(crate) fn div(a: F, b: F) -> F {
    ...
    #[inline(always)]
    const fn negate_u128(a: u128) -> u128 {
        (<i128>::wrapping_neg(a as i128)) as u128
    }
    ...
    if a_exponent.wrapping_sub(one) >= (max_exponent - 1) as u128
        || b_exponent.wrapping_sub(one) >= (max_exponent - 1) as u128
    {
    ...
    let written_exponent: i128 = a_exponent
        .wrapping_sub(b_exponent)
        .wrapping_add(scale as FInt)
        .wrapping_add(exponent_bias as FInt) as i128;
    ...
    let mut x_uq0 = if NUMBER_OF_HALF_ITERATIONS > 0 {
        ...
        // -                      [      b_UQ1       ]
        // = [      result       ][.... discarded ...]
        let corr_uq1 = negate_u128(
            (x_uq0_hw as FInt) * (b_uq1_hw as FInt) + (((x_uq0_hw as FInt) * (blo)) >> hw) - 1,
        ); // account for *possible* carry
        let lo_corr = corr_uq1 & lo_mask;
        let hi_corr = corr_uq1 >> hw;
        // x_UQ0 * corr_UQ1 = (x_UQ0_hw * 2^HW) * (hi_corr * 2^HW + lo_corr) - corr_UQ1
        let mut x_uq0: FInt = (((x_uq0_hw as u128) * hi_corr) << 1)
            .wrapping_add(((x_uq0_hw as u128) * lo_corr) >> (hw - 1))
    ...
    let mut x_uq0 = if USE_NATIVE_FULL_ITERATIONS {
        ...
        while idx < NUMBER_OF_FULL_ITERATIONS {
            let corr_uq1: u128 = 0_u128
                .wrapping_sub(*(((u256::from(x_uq0)) * (u256::from(b_uq1))) >> F::BITS).low());
            x_uq0 = *(((u256::from(x_uq0)) * (u256::from(corr_uq1))) >> (F::BITS - 1)).low();
            idx += 1;
        }
        x_uq0
    } else {
    ...
    let mut quotient: FInt = u128_widen_mul(x_uq0, a_significand << 1).1;
    ...
    if written_exponent >= max_exponent as i128 {
        return F::from_repr(inf_rep | quotient_sign);
    }
    ...
    let abs_result = if written_exponent > 0 {
        ...
        ret |= (written_exponent as u128) << significand_bits;
        residual <<= 1;
        ret
    } else {
        if (significand_bits as i128 + written_exponent) < 0 {
            return F::from_repr(quotient_sign);
        }
        let ret = quotient.wrapping_shr((negate_u128(written_exponent as u128) + 1) as u32);
